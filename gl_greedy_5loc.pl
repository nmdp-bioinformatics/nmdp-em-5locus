...
$glstring = "DRB1*UUUU+DRB1*UUUU";
$GLID_locus{"4444444444"} = "DRB1";
$GLID{"4444444444"} = [ @genos ];
$numglids{$GLID_locus{"4444444444"}}++;
...
sub loadDonors {
  ...
  while (<$fh_input>) {
    ...
    my ($id,$race,$ethnicity,$glid_a,$glid_b,$glid_c,$glid_drb1,$glid_dqb1) = split/,/,$_;
    
    # minimum A B DRB1 - not a restriction for Yoram dataset
    ...
    if (($glid_a eq " ") || ($glid_c eq " ") || ($glid_b eq " ") || ($glid_drb1 eq " ") || ($glid_dqb1 eq " ")) {
      print STDERR "Donor is missing typing $id $glid_a $glid_c $glid_b $glid_drb1 $glid_dqb1\n";
      next; 
    }

    # rename dqb1 to "999999999" to avoid mixup with C
    # if ($glid_c == 0) { $glid_c = 1; }
    if ($glid_drb1 == 0) { $glid_drb1 = 4444444444; }
    ...
    # $$rID{$id} = join(":",$glid_a,$glid_c,$glid_b,$glid_drbx,$glid_drb1,$glid_dqa1,$glid_dqb1,$glid_dpa1,$glid_dpb1);
    $$rID{$id} = join(":",$glid_a,$glid_c,$glid_b,$glid_drb1,$glid_dqb1);
    $$rGLID_count{$glid_a}++;
    $$rGLID_count{$glid_c}++;
    $$rGLID_count{$glid_b}++;
    # $$rGLID_count{$glid_drbx}++;
    $$rGLID_count{$glid_drb1}++;
    $$rGLID_count{$glid_dqb1}++;
    # $$rGLID_count{$glid_dqa1}++;
    # $$rGLID_count{$glid_dpa1}++;
    # $$rGLID_count{$glid_dpb1}++;
...
sub loadAlleleLists {
  ...
  foreach my $locus (@locilist) {
    ...
    if ($common_allele_seeding) {
      ...
      my $allele_filename = "";
      $allele_filename = $cfg_dir."/common_alleles_".$s_pop."_" . $locus . ".cfg";
...
sub addAlleleSingleHit {
  ...
  foreach my $id (keys %$rID) {
    ...
    # genomic order
    my($glid_a,$glid_c,$glid_b,$glid_drb1,$glid_dqb1) = split /:/,$glids;
    #print STDERR "GLIDS: $glids\n";
    if (!exists($$rGLID_hits{$glid_a}) || !exists($$rGLID_hits{$glid_c}) || !exists($$rGLID_hits{$glid_b}) || 
      !exists($$rGLID_hits{$glid_drb1}) || !exists ($$rGLID_hits{$glid_dqb1})) {
      #print STDERR "$id $glids\n";
    }

    if(!defined $$rGLID_hits{$glid_a}){
      print STDERR "A GLID not defined: ID $id GLID $glid_a\n";
    }
    if(!defined $$rGLID_hits{$glid_c}){
      print STDERR "C GLID not defined: ID $id GLID $glid_c\n";
    }    
    if(!defined $$rGLID_hits{$glid_b}){
      print STDERR "B GLID not defined: ID $id GLID $glid_b\n";
    }    
    if(!defined $$rGLID_hits{$glid_drb1}){
      print STDERR "DRB1 GLID not defined: ID $id GLID $glid_drb1\n";
    }    
    if(!defined $$rGLID_hits{$glid_dqb1}){
      print STDERR "DQB1 GLID not defined: ID $id GLID $glid_dqb1\n";
    }


    if (($$rGLID_hits{$glid_a} == 0)    && ($$rGLID_hits{$glid_c} == 0)   && ($$rGLID_hits{$glid_b} == 0) &&  
        ($$rGLID_hits{$glid_drb1} == 0) && ($$rGLID_hits{$glid_dqb1} == 0)) {
      $total_valid_donors++;
    }
    else {
...
sub loadBlankGLstring {
  ...
  foreach my $loctyp1 (keys %$rALL) {
    my ($loc1,$typ1) = split /\*/,$loctyp1;
    if ($loc1 ne "DRB1") { next; }
    if ($typ1 eq "UUUU") { next; }
    foreach my $loctyp2 (keys %$rALL) {
      my ($loc2,$typ2) = split /\*/,$loctyp2;
      if ($loc2 ne "DRB1") { next; }
      if ($typ2 eq "UUUU") { next; }
      my $geno = "$loctyp1+$loctyp2";
      push (@newgenos_DQB1,$geno);
    }
  }
  $$rGLID_reduced{"4444444444"} = [ @newgenos_DQB1 ];
...
sub outputGL {
  ...
  my $glid_filename = $output_dir."/glid.$nemo_race.txt";
  ...
  my $donor_filename = $output_dir."/pull.$nemo_race.txt";

  print STDERR "Printing reduced donor file $donor_filename\n";

  open (my $fh_donor,">","$donor_filename");
  foreach my $id (keys %$rID) {
    if (exists $$rdoublehits{$id}) { next; }  # skip doublehits
    ...
    my $newglids = join ":", map{ $GLID_number{$GLID_glstring{$_}}  } (split(/:/,$$rID{$id}));
    print $fh_donor "$id,$newglids\n";
  }
  close $fh_donor;

  # output donor file with doublehits
  my $doublehit_filename = $output_dir."/doublehit.$nemo_race.txt";
  ...
  my $uninterp_filename = $output_dir."/uninterp.$nemo_race.txt";
  ...
  foreach my $locus (@locilist) {
    ...
    my $antigens_filename = $output_dir."antigens.$nemo_race.$locus.txt";
  ...
  my $allelesets_filename = $output_dir."allelesets.$nemo_race.txt";
